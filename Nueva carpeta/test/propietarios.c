...
void inicializarPropietariosHardCode(ePropietario listadoPropietarios[])
{
    ...
    for(i=0; i<4; i++)
    {
        ...
        strcpy(listadoPropietarios[i].edad,edad[i]);
    }
}
...
void ePropietario_mostrarListado(ePropietario listadoPropietarios[],int limite)
{
    ...
    printf("ID -- Nombre -- Direccion -- Tarjeta-- edad");
        for(i=0; i<limite; i++)
        {
            if(listadoPropietarios[i].estado==OCUPADO)
            {
...
void ePropietario_mostrarListadoAscendente(ePropietario  propi[] , listadoPropietarios, char nombreApellido)
{
    ePropietario aux;
    for(int i=0;i<TAM-1;i++)
        {
            ...
            {
            if(stricmp(propi[j].nombreApellido,propi[i].nombreApellido)<0)
                {
                    aux=propi[i];
                    propi[i]=propi[j];
                    propi[j]=aux;
