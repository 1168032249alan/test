...
void eAuto_ingreso(eAuto listadoAutos[],ePropietario listadoPropietarios[], int largoAutos, int largoPropietarios)
{
    int idUsuario, indiceVacio, i, flag=0, idAutito;
        ...
        printf("\nIngrese ID de propietario para  ingresar el auto: ");
        scanf("%d",&idUsuario);
        ...
        if(flag==0)
        {
            printf("No existe el propietario.\n");
        }

        if(indiceVacio<0)
        {
            printf("No hay mas espacio , LO 100to.\n");
        }
}
...
{
    int i,j, ultimoOcupado, auxi, indicePropietario,idProp;
